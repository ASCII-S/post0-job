---
created: '2025-11-16'
last_reviewed: '2025-11-16'
next_review: '2025-11-16'
review_count: 0
difficulty: medium
mastery_level: 0.0
tags:
- 熟悉python
- 熟悉python/常用字符串方法的时间复杂度.md
related_outlines: []
---

# 常用字符串方法的时间复杂度

## 面试标准答案（可背诵）

Python 字符串的常用方法时间复杂度如下：**索引访问和切片**是 O(k)，k 为切片长度；**查找方法**（find、index、in）是 O(n)，使用线性搜索；**替换方法**（replace）是 O(n)，需要遍历整个字符串；**分割和连接**（split、join）是 O(n)；**大小写转换**（upper、lower）是 O(n)；**去除空白**（strip）是 O(n)。需要注意的是，Python 字符串是**不可变对象**，任何修改操作都会创建新字符串，因此频繁拼接应使用 `join()` 而非 `+` 操作符，以避免 O(n²) 的时间复杂度。

---

## 详细讲解

### 1. Python 字符串的底层实现

#### 1.1 字符串的不可变性

```python
# Python 字符串是不可变对象（immutable）
s = "hello"
# s[0] = 'H'  # TypeError: 'str' object does not support item assignment

# 任何"修改"操作都会创建新字符串
s2 = s.upper()  # 创建新字符串 "HELLO"
print(id(s), id(s2))  # 不同的内存地址
```

**底层结构（CPython）：**
```c
typedef struct {
    PyObject_HEAD
    Py_ssize_t length;          // 字符串长度
    Py_hash_t hash;             // 缓存的哈希值
    struct {
        unsigned int interned:2;  // 字符串驻留标志
        unsigned int kind:3;      // 编码类型（ASCII/Latin1/UCS2/UCS4）
        unsigned int compact:1;
        unsigned int ascii:1;
        unsigned int ready:1;
    } state;
    wchar_t *wstr;              // 字符数据
} PyUnicodeObject;
```

#### 1.2 字符串编码

```python
# Python 3 字符串使用 Unicode
# 根据内容选择最优编码方式

# ASCII (1 byte per char)
s1 = "hello"

# Latin-1 (1 byte per char)
s2 = "café"

# UCS-2 (2 bytes per char)
s3 = "你好"

# UCS-4 (4 bytes per char)
s4 = "𝕳𝖊𝖑𝖑𝖔"  # 数学字母

import sys
print(sys.getsizeof(s1))  # 较小
print(sys.getsizeof(s4))  # 较大
```

### 2. 访问和切片操作

#### 2.1 索引访问

| 操作     | 时间复杂度 | 说明             |
| -------- | ---------- | ---------------- |
| `s[i]`   | O(1)       | 直接通过索引访问 |
| `s[-1]`  | O(1)       | 负索引也是 O(1)  |
| `len(s)` | O(1)       | 长度已缓存       |

```python
s = "hello world"

# O(1) - 索引访问
char = s[0]      # 'h'
last = s[-1]     # 'd'

# O(1) - 获取长度
length = len(s)  # 11

# 原理：字符串长度存储在对象头部
# 索引访问通过 base_address + index * char_size 计算
```

#### 2.2 切片操作

| 操作     | 时间复杂度 | 说明                         |
| -------- | ---------- | ---------------------------- |
| `s[i:j]` | O(k)       | k = j - i，需要复制 k 个字符 |
| `s[:]`   | O(n)       | 复制整个字符串               |
| `s[::2]` | O(n)       | 需要遍历并复制               |

```python
s = "hello world"

# O(k) - 切片，k 为切片长度
sub1 = s[0:5]    # "hello" - O(5)
sub2 = s[6:]     # "world" - O(5)
sub3 = s[::2]    # "hlowrd" - O(11)，需要遍历

# 切片创建新字符串
print(id(s), id(sub1))  # 不同地址

# 性能测试
import timeit

def slice_short():
    return "a" * 1000000 [0:10]

def slice_long():
    return "a" * 1000000 [0:500000]

print(timeit.timeit(slice_short, number=10000))  # 快
print(timeit.timeit(slice_long, number=10000))   # 慢
```

### 3. 查找和搜索方法

#### 3.1 查找方法时间复杂度

| 方法                   | 时间复杂度 | 说明                     |
| ---------------------- | ---------- | ------------------------ |
| `s.find(sub)`          | O(n*m)     | n=字符串长度，m=子串长度 |
| `s.index(sub)`         | O(n*m)     | 同 find，未找到抛异常    |
| `sub in s`             | O(n*m)     | 使用优化的搜索算法       |
| `s.count(sub)`         | O(n*m)     | 需要遍历整个字符串       |
| `s.startswith(prefix)` | O(m)       | m=前缀长度               |
| `s.endswith(suffix)`   | O(m)       | m=后缀长度               |

```python
s = "hello world hello python"

# O(n*m) - 查找子串
pos1 = s.find("world")        # 6
pos2 = s.find("java")         # -1 (未找到)

# O(n*m) - index 方法（未找到会抛异常）
try:
    pos3 = s.index("python")  # 18
    pos4 = s.index("java")    # ValueError
except ValueError:
    pass

# O(n*m) - in 操作符
exists = "hello" in s         # True

# O(n*m) - 计数
count = s.count("hello")      # 2

# O(m) - 前缀/后缀检查
is_start = s.startswith("hello")  # True - O(5)
is_end = s.endswith("python")     # True - O(6)
```

#### 3.2 查找算法优化

```python
# CPython 使用优化的字符串搜索算法
# 对于短字符串：Brute Force
# 对于长字符串：Boyer-Moore-Horspool 或类似算法

# 性能对比
import timeit

text = "a" * 1000000 + "b"

# 最坏情况：O(n*m)
def worst_case():
    return text.find("aaab")

# 最好情况：O(n)
def best_case():
    return text.find("b")

print(timeit.timeit(worst_case, number=100))  # 较慢
print(timeit.timeit(best_case, number=100))   # 较快
```

### 4. 修改和转换方法

#### 4.1 大小写转换

| 方法             | 时间复杂度 | 说明         |
| ---------------- | ---------- | ------------ |
| `s.upper()`      | O(n)       | 遍历所有字符 |
| `s.lower()`      | O(n)       | 遍历所有字符 |
| `s.capitalize()` | O(n)       | 遍历所有字符 |
| `s.title()`      | O(n)       | 遍历所有字符 |
| `s.swapcase()`   | O(n)       | 遍历所有字符 |

```python
s = "Hello World"

# O(n) - 大小写转换，创建新字符串
upper = s.upper()        # "HELLO WORLD"
lower = s.lower()        # "hello world"
title = s.title()        # "Hello World"
swap = s.swapcase()      # "hELLO wORLD"

# 每次都创建新字符串
print(id(s), id(upper), id(lower))  # 三个不同地址

# 性能测试
import timeit

short_str = "hello"
long_str = "hello" * 100000

print(timeit.timeit(lambda: short_str.upper(), number=100000))  # 快
print(timeit.timeit(lambda: long_str.upper(), number=100000))   # 慢
```

#### 4.2 替换方法

| 方法                         | 时间复杂度 | 说明                    |
| ---------------------------- | ---------- | ----------------------- |
| `s.replace(old, new)`        | O(n*m)     | n=字符串长度，m=old长度 |
| `s.replace(old, new, count)` | O(n*m)     | 限制替换次数            |

```python
s = "hello world hello python"

# O(n*m) - 替换所有匹配
s1 = s.replace("hello", "hi")  # "hi world hi python"

# O(n*m) - 限制替换次数
s2 = s.replace("hello", "hi", 1)  # "hi world hello python"

# 替换空字符串（删除）
s3 = s.replace(" ", "")  # "helloworldhellopython"

# 性能分析
def replace_test():
    text = "a" * 1000000
    return text.replace("a", "b")  # O(n)

import timeit
print(timeit.timeit(replace_test, number=10))
```

### 5. 分割和连接方法

#### 5.1 分割方法

| 方法                     | 时间复杂度 | 说明             |
| ------------------------ | ---------- | ---------------- |
| `s.split()`              | O(n)       | 按空白分割       |
| `s.split(sep)`           | O(n)       | 按指定分隔符分割 |
| `s.split(sep, maxsplit)` | O(n)       | 限制分割次数     |
| `s.rsplit()`             | O(n)       | 从右侧分割       |
| `s.splitlines()`         | O(n)       | 按行分割         |
| `s.partition(sep)`       | O(n)       | 分割为三部分     |

```python
s = "hello world python programming"

# O(n) - 按空白分割
words = s.split()  # ['hello', 'world', 'python', 'programming']

# O(n) - 按指定分隔符
csv = "a,b,c,d"
items = csv.split(",")  # ['a', 'b', 'c', 'd']

# O(n) - 限制分割次数
limited = s.split(" ", 2)  # ['hello', 'world', 'python programming']

# O(n) - 从右侧分割
right = s.rsplit(" ", 1)  # ['hello world python', 'programming']

# O(n) - 分割为三部分（只分割一次）
before, sep, after = s.partition("world")
# ('hello ', 'world', ' python programming')

# 多行文本
text = "line1\nline2\nline3"
lines = text.splitlines()  # ['line1', 'line2', 'line3']
```

#### 5.2 连接方法

| 方法                 | 时间复杂度 | 说明               |
| -------------------- | ---------- | ------------------ |
| `sep.join(iterable)` | O(n)       | n=所有字符串总长度 |
| `s1 + s2`            | O(n+m)     | 创建新字符串       |
| `s1 += s2` (循环中)  | O(n²)      | 每次创建新字符串   |

```python
# O(n) - join 方法（推荐）
words = ['hello', 'world', 'python']
result1 = " ".join(words)  # "hello world python"

# join 只遍历一次，预先计算总长度
# 时间复杂度：O(n)，n 为所有字符串总长度

# O(n²) - 循环中使用 + 拼接（不推荐）
result2 = ""
for word in words:
    result2 += word + " "  # 每次创建新字符串

# 性能对比
import timeit

def join_method():
    words = ["hello"] * 10000
    return " ".join(words)

def plus_method():
    words = ["hello"] * 10000
    result = ""
    for word in words:
        result += word + " "
    return result

print("join:", timeit.timeit(join_method, number=100))   # 快
print("plus:", timeit.timeit(plus_method, number=100))   # 慢很多
```

**为什么 join 比 + 快？**

```python
# + 操作符在循环中的问题
result = ""
for i in range(n):
    result += str(i)  # 每次都创建新字符串
    
# 第 1 次：创建长度 1 的字符串
# 第 2 次：创建长度 2 的字符串（复制前 1 个字符）
# 第 3 次：创建长度 3 的字符串（复制前 2 个字符）
# ...
# 总复制次数：1 + 2 + 3 + ... + n = O(n²)

# join 方法
result = "".join(str(i) for i in range(n))
# 1. 遍历一次计算总长度：O(n)
# 2. 分配一次内存：O(1)
# 3. 复制所有字符：O(n)
# 总时间：O(n)
```

### 6. 去除和填充方法

#### 6.1 去除空白

| 方法             | 时间复杂度 | 说明         |
| ---------------- | ---------- | ------------ |
| `s.strip()`      | O(n)       | 去除两端空白 |
| `s.lstrip()`     | O(n)       | 去除左侧空白 |
| `s.rstrip()`     | O(n)       | 去除右侧空白 |
| `s.strip(chars)` | O(n)       | 去除指定字符 |

```python
s = "  hello world  \n"

# O(n) - 去除两端空白
clean = s.strip()    # "hello world"

# O(n) - 去除左侧
left = s.lstrip()    # "hello world  \n"

# O(n) - 去除右侧
right = s.rstrip()   # "  hello world"

# O(n) - 去除指定字符
s2 = "***hello***"
clean2 = s2.strip("*")  # "hello"

# 实现原理：从两端向中间扫描
def my_strip(s):
    # 找到第一个非空白字符
    start = 0
    while start < len(s) and s[start].isspace():
        start += 1
    
    # 找到最后一个非空白字符
    end = len(s) - 1
    while end >= start and s[end].isspace():
        end -= 1
    
    return s[start:end+1]
```

#### 6.2 填充方法

| 方法              | 时间复杂度 | 说明       |
| ----------------- | ---------- | ---------- |
| `s.ljust(width)`  | O(n)       | 左对齐填充 |
| `s.rjust(width)`  | O(n)       | 右对齐填充 |
| `s.center(width)` | O(n)       | 居中填充   |
| `s.zfill(width)`  | O(n)       | 用 0 填充  |

```python
s = "hello"

# O(n) - 左对齐（右侧填充）
left = s.ljust(10)       # "hello     "
left2 = s.ljust(10, "*") # "hello*****"

# O(n) - 右对齐（左侧填充）
right = s.rjust(10)      # "     hello"
right2 = s.rjust(10, "*")# "*****hello"

# O(n) - 居中
center = s.center(10)    # "  hello   "

# O(n) - 数字填充 0
num = "42"
padded = num.zfill(5)    # "00042"
```

### 7. 检查和验证方法

#### 7.1 内容检查

| 方法          | 时间复杂度 | 说明                   |
| ------------- | ---------- | ---------------------- |
| `s.isalpha()` | O(n)       | 检查是否全是字母       |
| `s.isdigit()` | O(n)       | 检查是否全是数字       |
| `s.isalnum()` | O(n)       | 检查是否全是字母或数字 |
| `s.isspace()` | O(n)       | 检查是否全是空白       |
| `s.isupper()` | O(n)       | 检查是否全是大写       |
| `s.islower()` | O(n)       | 检查是否全是小写       |

```python
# O(n) - 所有检查方法都需要遍历字符串

s1 = "hello"
print(s1.isalpha())   # True
print(s1.isdigit())   # False

s2 = "12345"
print(s2.isdigit())   # True
print(s2.isalnum())   # True

s3 = "hello123"
print(s3.isalnum())   # True
print(s3.isalpha())   # False

# 实现原理
def my_isdigit(s):
    if not s:
        return False
    for char in s:  # O(n)
        if not '0' <= char <= '9':
            return False
    return True
```

### 8. 编码和解码

#### 8.1 编码方法

| 方法             | 时间复杂度 | 说明         |
| ---------------- | ---------- | ------------ |
| `s.encode()`     | O(n)       | 字符串转字节 |
| `bytes.decode()` | O(n)       | 字节转字符串 |

```python
# O(n) - 编码
s = "你好世界"
utf8_bytes = s.encode('utf-8')      # b'\xe4\xbd\xa0\xe5\xa5\xbd...'
gbk_bytes = s.encode('gbk')         # 不同编码

# O(n) - 解码
decoded = utf8_bytes.decode('utf-8')  # "你好世界"

# 不同编码的字节长度不同
print(len(s))           # 4 (字符数)
print(len(utf8_bytes))  # 12 (UTF-8: 每个中文3字节)
print(len(gbk_bytes))   # 8 (GBK: 每个中文2字节)
```

### 9. 格式化方法

#### 9.1 格式化时间复杂度

| 方法         | 时间复杂度 | 说明                 |
| ------------ | ---------- | -------------------- |
| `s.format()` | O(n)       | 格式化字符串         |
| `f-string`   | O(n)       | 格式化字符串（推荐） |
| `%` 格式化   | O(n)       | 旧式格式化           |

```python
# O(n) - format 方法
name = "Alice"
age = 30
s1 = "Name: {}, Age: {}".format(name, age)

# O(n) - f-string（Python 3.6+，推荐）
s2 = f"Name: {name}, Age: {age}"

# O(n) - % 格式化（旧式）
s3 = "Name: %s, Age: %d" % (name, age)

# 性能对比
import timeit

def format_method():
    return "Hello {}".format("world")

def f_string_method():
    world = "world"
    return f"Hello {world}"

def percent_method():
    return "Hello %s" % "world"

print("format:", timeit.timeit(format_method, number=1000000))
print("f-string:", timeit.timeit(f_string_method, number=1000000))  # 最快
print("percent:", timeit.timeit(percent_method, number=1000000))
```

### 10. 时间复杂度总结表

#### 10.1 完整时间复杂度表

| 操作类别 | 方法             | 时间复杂度 | 空间复杂度 |
| -------- | ---------------- | ---------- | ---------- |
| **访问** | `s[i]`           | O(1)       | O(1)       |
|          | `len(s)`         | O(1)       | O(1)       |
|          | `s[i:j]`         | O(k)       | O(k)       |
| **查找** | `s.find(sub)`    | O(n*m)     | O(1)       |
|          | `sub in s`       | O(n*m)     | O(1)       |
|          | `s.count(sub)`   | O(n*m)     | O(1)       |
|          | `s.startswith()` | O(m)       | O(1)       |
| **转换** | `s.upper()`      | O(n)       | O(n)       |
|          | `s.lower()`      | O(n)       | O(n)       |
|          | `s.replace()`    | O(n*m)     | O(n)       |
| **分割** | `s.split()`      | O(n)       | O(n)       |
|          | `sep.join()`     | O(n)       | O(n)       |
| **去除** | `s.strip()`      | O(n)       | O(n)       |
| **检查** | `s.isdigit()`    | O(n)       | O(1)       |
| **拼接** | `s1 + s2`        | O(n+m)     | O(n+m)     |
|          | `s += t` (循环)  | O(n²)      | O(n²)      |

### 11. 性能优化建议

#### 11.1 字符串拼接优化

```python
# ❌ 不推荐：循环中使用 +
def bad_concat(items):
    result = ""
    for item in items:
        result += str(item)  # O(n²)
    return result

# ✅ 推荐：使用 join
def good_concat(items):
    return "".join(str(item) for item in items)  # O(n)

# ✅ 推荐：使用列表累积
def good_concat2(items):
    parts = []
    for item in items:
        parts.append(str(item))
    return "".join(parts)  # O(n)

# ✅ 推荐：使用 io.StringIO
from io import StringIO

def good_concat3(items):
    buffer = StringIO()
    for item in items:
        buffer.write(str(item))
    return buffer.getvalue()  # O(n)

# 性能测试
import timeit

items = range(10000)
print("bad:", timeit.timeit(lambda: bad_concat(items), number=10))
print("good:", timeit.timeit(lambda: good_concat(items), number=10))
```

#### 11.2 字符串查找优化

```python
# 多次查找同一子串
text = "hello world " * 1000

# ❌ 不推荐：多次调用 find
def multiple_finds():
    positions = []
    for i in range(100):
        pos = text.find("world")
        positions.append(pos)
    return positions

# ✅ 推荐：缓存结果
def cached_find():
    pos = text.find("world")
    return [pos] * 100

# ✅ 推荐：使用正则表达式（复杂模式）
import re

def regex_find():
    pattern = re.compile(r"world")
    return [m.start() for m in pattern.finditer(text)]
```

#### 11.3 字符串比较优化

```python
# 大小写不敏感比较

# ❌ 不推荐：每次都转换
def bad_compare(s1, s2):
    return s1.lower() == s2.lower()  # 每次都创建新字符串

# ✅ 推荐：使用 casefold（更准确）
def good_compare(s1, s2):
    return s1.casefold() == s2.casefold()

# ✅ 推荐：先比较长度
def better_compare(s1, s2):
    if len(s1) != len(s2):  # O(1)
        return False
    return s1.casefold() == s2.casefold()  # O(n)

# 前缀/后缀检查
# ✅ 使用专门方法
if text.startswith("hello"):  # O(m)，m 为前缀长度
    pass

# ❌ 不要使用切片比较
if text[:5] == "hello":  # O(5)，但创建了新字符串
    pass
```

#### 11.4 字符串构建优化

```python
# 构建大型字符串

# ❌ 不推荐
def build_html_bad():
    html = "<html>"
    html += "<body>"
    html += "<h1>Title</h1>"
    # ... 更多内容
    html += "</body>"
    html += "</html>"
    return html

# ✅ 推荐：使用列表
def build_html_good():
    parts = [
        "<html>",
        "<body>",
        "<h1>Title</h1>",
        # ... 更多内容
        "</body>",
        "</html>"
    ]
    return "".join(parts)

# ✅ 推荐：使用模板
from string import Template

def build_html_template():
    template = Template("""
    <html>
        <body>
            <h1>$title</h1>
        </body>
    </html>
    """)
    return template.substitute(title="Title")
```

### 12. 实际应用场景

#### 12.1 日志处理

```python
# 场景：处理大量日志行

# ❌ 低效方式
def process_logs_bad(log_file):
    result = ""
    with open(log_file) as f:
        for line in f:
            if "ERROR" in line:  # O(n)
                result += line   # O(n²)
    return result

# ✅ 高效方式
def process_logs_good(log_file):
    errors = []
    with open(log_file) as f:
        for line in f:
            if "ERROR" in line:
                errors.append(line)
    return "".join(errors)  # O(n)

# ✅ 更好：使用生成器
def process_logs_best(log_file):
    with open(log_file) as f:
        return "".join(line for line in f if "ERROR" in line)
```

#### 12.2 CSV 处理

```python
# 场景：解析 CSV 数据

# ❌ 手动分割（不推荐）
def parse_csv_bad(csv_string):
    lines = csv_string.split("\n")  # O(n)
    result = []
    for line in lines:
        fields = line.split(",")     # O(m)
        result.append(fields)
    return result

# ✅ 使用 csv 模块
import csv
from io import StringIO

def parse_csv_good(csv_string):
    return list(csv.reader(StringIO(csv_string)))
```

#### 12.3 URL 处理

```python
# 场景：构建 URL 查询字符串

# ❌ 手动拼接
def build_query_bad(params):
    query = "?"
    for key, value in params.items():
        query += f"{key}={value}&"
    return query[:-1]  # 去除最后的 &

# ✅ 使用 urllib
from urllib.parse import urlencode

def build_query_good(params):
    return "?" + urlencode(params)

# 示例
params = {"name": "Alice", "age": 30, "city": "NYC"}
print(build_query_good(params))
# ?name=Alice&age=30&city=NYC
```

---

## 总结

### 核心要点

1. **基本操作复杂度**：
   - 索引访问：O(1)
   - 切片：O(k)
   - 查找：O(n*m)
   - 修改：O(n)（创建新字符串）

2. **字符串不可变性**：
   - 任何修改都创建新字符串
   - 循环中使用 `+=` 会导致 O(n²)

3. **拼接优化**：
   - 使用 `join()` 而非 `+`
   - 使用列表累积再 join
   - 使用 `io.StringIO` 处理大量拼接

4. **查找优化**：
   - 缓存重复查找结果
   - 使用 `startswith/endswith` 检查前后缀
   - 复