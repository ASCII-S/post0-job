# 📅 复习清单

**生成时间**: 2025-11-22

## 统计概览

- 🔴 **已过期**: 10 篇 (共433篇，显示前10篇)
- ⭐ **reviewsToday**: 1 篇
- 📅 **本周计划**: 2 篇
- 📆 **未来安排**: 0 篇

## 🔴 已过期（优先复习）

_按智能排序，从易到难，建议从上往下复习_

- [ ] [字符串切片的底层实现原理](notes/熟悉python/字符串切片的底层实现原理.md)
  - 已复习: 1次 | 难度: medium 
- [ ] [格式化字符串的几种方式：%、format()、f-string的对比](notes/熟悉python/格式化字符串的几种方式：%、format()、f-string的对比.md)
  - 已复习: 1次 | 难度: medium 
- [ ] [列表推导式与生成器表达式的区别](notes/熟悉python/列表推导式与生成器表达式的区别.md)
  - 已复习: 0次 | 难度: medium 
- [ ] [CLAUDE](notes/熟悉python/CLAUDE.md)
  - 已复习: 0次 | 难度: medium 
- [ ] [列表的时间复杂度：append、insert、pop、index](notes/熟悉python/列表的时间复杂度：append、insert、pop、index.md)
  - 已复习: 0次 | 难度: medium 
- [ ] [Linux中的各种安装包有什么？](notes/_seeds/待分类/Linux中的各种安装包有什么？.md)
  - 已复习: 0次 | 难度: medium 
- [ ] [如何保持Linux内核和软件的版本稳定性如何维护](notes/_seeds/待分类/如何保持Linux内核和软件的版本稳定性如何维护.md)
  - 已复习: 0次 | 难度: medium 
- [ ] [Linux的各种版本是什么？](notes/_seeds/待分类/Linux的各种版本是什么？.md)
  - 已复习: 0次 | 难度: medium 
- [ ] [列表的底层实现原理（动态数组）](notes/熟悉python/列表的底层实现原理（动态数组）.md)
  - 已复习: 0次 | 难度: medium 
- [ ] [常用字符串方法的时间复杂度](notes/熟悉python/常用字符串方法的时间复杂度.md)
  - 已复习: 0次 | 难度: medium 

## ⭐ reviewsToday

_按智能排序，从易到难_

- [ ] [join()和+拼接字符串的性能差异](notes/熟悉python/join()和+拼接字符串的性能差异.md)
  - 已复习: 2次 | 难度: medium 

## 📅 本周计划

_按智能排序_

**1.** - [ ] [动态量化和静态量化的区别是什么？推理时应该选择哪种？](notes/熟悉大语言模型推理优化-技术层次/动态量化和静态量化的区别是什么？推理时应该选择哪种？.md) - 2025-11-25
  - 已复习: 4次 | 难度: medium
**2.** - [ ] [对称量化和非对称量化的区别是什么？各自适用什么场景？](notes/熟悉大语言模型推理优化-技术层次/对称量化和非对称量化的区别是什么？各自适用什么场景？.md) - 2025-11-25
  - 已复习: 4次 | 难度: medium

## 🎮 学习进展

### 🔥 连击系统
连续学习：**5天** 🔥

### 📊 主题掌握度

| 主题 | 等级 | 经验值 | 掌握情况 |
|------|------|--------|----------|
| 🌟 熟悉大语言模型推理优化-技术层次 | 初级 📖 | ████░░░░░░ 120/300 XP | 0/94 (0%) |
| 🌟 精通大模型压缩技术 | 入门 🌱 | █████████░ 90/100 XP | 0/14 (0%) |
| 🌟 _seeds | 入门 🌱 | ███░░░░░░░ 30/100 XP | 0/8 (0%) |
| 🌟 熟悉CPU架构 | 入门 🌱 | ██░░░░░░░░ 20/100 XP | 0/1 (0%) |
| 🌟 精通vllm源码 | 入门 🌱 | ██░░░░░░░░ 20/100 XP | 0/3 (0%) |
| 🌟 C++ | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/72 (0%) |
| 🌟 DS | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/0 (0%) |
| 🌟 gemm-opt | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/5 (0%) |
| 🌟 cuda | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/77 (0%) |
| 🌟 pytorch | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/4 (0%) |
| 🌟 深度学习 | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/11 (0%) |
| 🌟 vllm | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/27 (0%) |
| 🌟 分布式通信 | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/6 (0%) |
| 🌟 test | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/0 (0%) |
| 🌟 操作系统 | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/2 (0%) |
| 🌟 Transformer | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/36 (0%) |
| 🌟 LeetcodeHot100 | 入门 🌱 | ░░░░░░░░░░ 0/100 XP | 0/50 (0%) |

💡 **提示**: 复习笔记可获得经验值！首次掌握（80%+）额外获得50XP

---

---

## 📖 使用说明

### 💡 推荐方式：在清单中打勾，然后同步

1. **复习笔记时，直接在本文档中打勾**：
   - 将 `- [ ]` 改为 `- [x]`
   
2. **复习完成后，运行同步命令**：
```bash
python scripts/review_manager.py sync
# 或使用快捷命令: ./scripts/kb.sh sync
```

这会自动更新所有打勾笔记的元数据！

### 其他方式

#### 标记单个文档
```bash
python scripts/review_manager.py mark-done "notes/cuda/Bank冲突的概念.md"
```

#### 调整难度
```bash
python scripts/review_manager.py set-difficulty "notes/cuda/Bank冲突.md" hard
```

#### 重新生成清单
```bash
python scripts/review_manager.py today
```
